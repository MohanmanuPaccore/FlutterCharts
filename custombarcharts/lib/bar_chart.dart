  ...
  void initState() {
    ...
    scrollController.addListener(() {
      if (scrollController.position.maxScrollExtent ==
        ...
        if (scrollController.offset <= 51.0) {
          setState(() {
            isScrolling = false;
          });
        }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: LayoutBuilder(builder: (p0, constraints) {
        final chartWidth = max(60.0 * (widget.bars.length),
            ...
            SingleChildScrollView(
              ...
              child: Container(
                ...
                child: CustomPaint(
                  ...
                  painter: BarChartPainterThree(widget.bars, widget.maxValue, false),
                ),
              ),
            ),
            !isScrolling
              ...
              child: Container(
                ...
                padding: const EdgeInsets.only(right: 20),
                child: CustomPaint(
                  size: Size(20, 400), // Adjust the size as needed
                  painter: BarChartPainterThree(
                      widget.bars, widget.maxValue, true),
                ),
              ),
            ),
          ],
...
class BarChartPainterThree extends CustomPainter {
  final List<Bar> bars;
  final double maxValue;
  final bool showOnlyYaxis;
  final double xLabelPadding = 100;
  final double yLabelPadding = 40;
  final double axisWidth = 2;

  BarChartPainterThree(this.bars, this.maxValue, this.showOnlyYaxis);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (!showOnlyYaxis) {
      ...
      double averageXPosition = yLabelPadding +
          ((averageX) / maxValue) * (size.width - xLabelPadding);
      ...
      x += dashWidth + dashSpace) {
        ...
        path.moveTo(
            x + dashSpace, size.height - bottomMargin - averageXPosition);
      }

      canvas.drawPath(path, dashedLinePaint);

      final avgLabel = 'Average: ${averageX.toStringAsFixed(1)}';
      final avgLabelPainter = TextPainter(
        text: TextSpan(
          text: avgLabel,
          style: const TextStyle(
              fontSize: 12, color: Colors.black), // Adjust styling
        ),
        textDirection: TextDirection.ltr,
      );
